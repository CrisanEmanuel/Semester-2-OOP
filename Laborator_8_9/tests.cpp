...
void Tests::testeCreareContract() {
    ...
    service.updateService("b", "zz", 44, "zz", "zz");
    try{
        service.searchService("b");
        assert(false);
    }catch(RepoException& ex){
        assert(ex.getMesaj() == "Disciplina inexistenta!\n");
    }
    assert(service.getAllDisciplineFromContractService()[0].getDenumire() == "zz");
...
void Tests::testeUndo() {
    ...
    service.updateService("b", "zz", 312, "zz", "zz");
    assert(service.getAll()[1].getDenumire() == "zz");

    service.undoService();
    assert(service.getAll()[1].getDenumire() == "b");
}